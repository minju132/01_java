...
public class Application01 {
    ...
    public static void main(String[] args) {
         ...
         * 클래스 또한 부모클래스와 자식클래스로 역학을 나누어서 부모가 가지는 멤버를 자식이 물려받아
         ...
         * 특히 메서드 재정의(overriding)라는 기술을 이용해서 부모가 가진 메서드를 재정의 하는 것도 가능하다.
         *
         * 메서드 재정의(overriding)이란 무고가 가지는 메서드 선언부를 그대로 사용하면서
         * 자식클래스가 정의한 메서드대로 동작하도록 구현 몸체 부분을 새롭게 다시 작성하는 기술이다.
         * 메서드 재정의를 하면 메서드 호출할 시 재정의한 메서드가 우선으로 동작하게 된다.
         *
         * 이러한 상속이라는 기술을 사용하게 되면 얻게되는 이점은 크게 두 가지로 볼 수 있다.
         * 1. 새로운 클래스를 작성할 시 기존에 작성한 클래스를 재사용 할 수 있다.
         *   1-1. 재사용 시 생산성을 크게 향상 시킬 수 있다.(새롭게 작성하는거보다 빠르다.)
         ...
         *   상속구조가 복잡해 질수록 그 영향에 대한 예측이 힘들며 이러한 단점이 유지보수성 증가한다는 장점과는
         *   반대로 유지보수에 악영향을 미친다.
         * 2. 또한 부모클래스의 변경 또한 쉽지 않다. 자식클래스에서 중요하게 사용하는 기능인 경우
         * 부모클래스를 변경할 시 자식클래스에 모두 영향을 줄 수 있다.
         * 반대로 유지보수에 악영향을 미친다.
         * 3. 부모 클래스에서는 의미있었던 기능이 자식클래스에서는 무의미할 수 있다.
         *
         * 장점과 단점을 고려 했을 때,
         ...
         *
         * 적잘한 책임을 수행하는 객체 또한 그 객체만 수행할 수 있는 기능이라기 보다 역할의 관점으로 바라봐야한다.
