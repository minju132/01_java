...
public class EmployeeService {
    ...
    public ArrayList employeeViewAll() throws Exception {
        ...
        if(employees == null){
            throw new Exception("전화번호부 정보 조회실패");
        }
    ...
    public EmployeeDTO employeeFindByName(String name) throws Exception {
        ...
        if(emp == null){
            throw new Exception("전화번호 정보 조회실패");
        }
        return emp;
    }

    public String empInsert(EmployeeDTO emp) throws Exception {
        // 서비스는 아래와 같이 우리의 비즈니스 로직에 맞는 유효성을 검사한다.
        // 아래는 사원의 번호가 중복되는 것을 확인하고 만약 중복이라면 등록을 취소해야한다.
        EmployeeDTO findEmp = employeeRepository.empFindById(emp.getUser_name());
