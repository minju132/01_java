...
public interface Interproduct extends Serializable {
    ...
    * 반드시 선언과 동시에 초기화 해줘야 한다.
    ...
    /*
    * 인터페이스 안에 작성한 메서드는 묵시적으로 public abstract의 의미를 가진다.
    * 따라서 인터페이스의 메서드를 오버라이딩하는 경우 반드시 접근제한자를 public으로 해야 오버라이디이 가능하다.
    */
    void abMethod();
    // static 메서드는 작성이 가능하다.(JDK 1.8부터 추가)
    public static void staticMethod(){
        System.out.println("interProduct 클래스의 staticMethod 호출됨");
    }
    // default 키워드를 사용하면 non-statcic 메서드도 작성이 가능하다.
